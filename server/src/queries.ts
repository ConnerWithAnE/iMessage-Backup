...
export const chatHandleCreateQuery = `CREATE TABLE ${CHAT_HANDLE_JOIN} (chat_id INTEGER REFERENCES chat (ROWID) ON DELETE CASCADE, handle_id INTEGER REFERENCES handle (ROWID) ON DELETE CASCADE, UNIQUE(chat_id, handle_id))`;

export const chatMessageCreateQuery = `CREATE TABLE ${CHAT_MESSAGE_JOIN} (chat_id INTEGER REFERENCES chat (ROWID) ON DELETE CASCADE, message_id INTEGER REFERENCES message (ROWID) ON DELETE CASCADE, message_date INTEGER DEFAULT 0, PRIMARY KEY (chat_id, message_id))`;

export const messageAttachmentCreateQuery = `CREATE TABLE ${MESSAGE_ATTACHMENT_JOIN} (message_id INTEGER REFERENCES message (ROWID) ON DELETE CASCADE, attachment_id INTEGER REFERENCES attachment (ROWID) ON DELETE CASCADE, UNIQUE(message_id, attachment_id))`;
